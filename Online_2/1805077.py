...
class InventorySystem:
    def __init__(self, InFile, OutFile, numEvents=4):
        ...
        self.numTrials = 0
        self.totalTrialCost = 0.0
        
        self.policyWithLag = []
        self.policyWithoutLag = []

    def input(self):        
        with self.inFile as file:
            line = file.readline()
            self.numTrials = int(line)
    ...
    def reportHeader(self):
        ...
        self.outFile.write("--------------------------------------------------------------------------------------------------\n")
        self.outFile.write(" Policy          Avg_TC_With_Lag    Avg_TC_Without_Lag        Better?\n")
        self.outFile.write("--------------------------------------------------------------------------------------------------\n\n")
    ...
    def report(self):
        ...
        totalCost = avgOrderingCost + avgHoldingCost + avgShortageCost
        # self.outFile.write("totalCost: %.2f\n" % (totalCost))
        self.totalTrialCost += totalCost
        # self.outFile.write("totalTrialCost: %.2f\n" % (self.totalTrialCost))
    ...
    def simulate(self):
        for i in range(self.numPolicies):
            ...
            for trial in range(self.numTrials):
                self.simulatePolicy(self.policies[i][0], self.policies[i][1], withLag=True)
                
            self.policyWithLag.append(self.totalTrialCost / self.numTrials)
            str = "(%2d,%3d) %19.2f" % (self.smalls, self.bigs, self.totalTrialCost / self.numTrials)
            
            self.totalTrialCost = 0.0
            for trial in range(self.numTrials):
                self.simulatePolicy(self.policies[i][0], self.policies[i][1], withLag=False)
            self.policyWithoutLag.append(self.totalTrialCost / self.numTrials)
            
            str += " %19.2f " % (self.totalTrialCost / self.numTrials)
            
            if (self.policyWithLag[i] < self.policyWithoutLag[i]):
                str += "                 With_Lag\n"
            else: 
                str += "              Without_Lag\n"
        ...
        optimalPolicy = self.policyWithLag.index(min(self.policyWithLag))
        self.outFile.write("Optimal policy(s):\n\n")
        self.outFile.write("[With Lag]: (%d, %d)\n" % (self.policies[optimalPolicy][0], self.policies[optimalPolicy][1]))
        
        optimalPolicy = self.policyWithoutLag.index(min(self.policyWithoutLag))
        self.outFile.write("[Without Lag]: (%d, %d)\n" % (self.policies[optimalPolicy][0], self.policies[optimalPolicy][1]))

    def simulatePolicy(self, s, S, withLag):
        self.smalls = s
        self.bigs = S
        self.initialize(withLag=withLag)
    ...
    def evaluate(self):
        if self.invLevel < self.smalls:
            ...
            if (self.withLag):
                ...
                self.timeNextEvent[ARRIVAL] = self.simTime
